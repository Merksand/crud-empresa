...
export default function Sucursales() {
  ...
  const [geolocalizaciones, setGeolocalizaciones] = useState([]); // Nueva lista de geolocalizaciones
  ...
  useEffect(() => {
    fetchSucursales();
    fetchMunicipios();
    fetchGeolocalizaciones(); // Cargar geolocalizaciones al inicio
  }, []);
  ...
  const fetchGeolocalizaciones = async () => {
    try {
      const response = await fetch('/api/geolocalizacion');   
      const data = await response.json();
      setGeolocalizaciones(data);
    } catch (error) {
      console.error('Error al cargar geolocalizaciones:', error);
      showNotification('Error al cargar las geolocalizaciones', 'error');
    }
  };
    ...
    <div className="p-6">
                ...
                sucursales.map((sucursal) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      {sucursal.Latitud_Geo && sucursal.Longitud_Geo
                        ? `Lat: ${sucursal.Latitud_Geo}, Lon: ${sucursal.Longitud_Geo}`
                        : 'Sin Geolocalización'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm">{sucursal.Nombre_Parametro_Suc}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <span
                        className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${sucursal.Estado_Suc === 'Activo'
      ...
      >
        <SucursalForm
          ...
          municipios={municipios} // Pasar lista de municipios
          geolocalizaciones={geolocalizaciones} // Pasar lista de geolocalizaciones
          onSubmit={async (data) => {
            try {
              if (sucursalEditar) {
